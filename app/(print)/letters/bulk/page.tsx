...
function BulkInner() {
  ...
  useEffect(() => {
    if (!ids.length) { setStatus('No letters selected.'); return }
    let cancelled = false
    ;(async () => {
      const parts: string[] = []
      for (let i = 0; i < ids.length; i++) {
        ...
        try {
          const res = await fetch(`/api/letters/${ids[i]}/html`)
          if (res.ok) {
            const t = await res.text()
            const m = t.match(/<body[^>]*>([\s\S]*)<\/body>/i)
            parts.push(`<div class="letter-page">${m ? m[1] : t}</div>`)
          }
        } catch {}
      }
      if (cancelled) return
  ...
  return (
    <>
      <style>{`
        ...
        .letter-page { page-break-after: always; break-after: page; }
...
export default function BulkLetterPrintPage() {
